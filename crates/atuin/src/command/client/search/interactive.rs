...
fn restore_popup_area(saved: &SavedScreen, popup_rect: Rect, scroll_offset: u16) {
    ...
    // rows_formatted(start_col, width) returns all visible rows formatted
    // with ANSI escapes, restricted to the given column range. Collect once
    // so we can index by source row.
    ...
    for dy in 0..popup_rect.height {
        ...
        // Clear the entire line first. rows_formatted() skips default cells
        // (spaces with default attributes) using cursor jumps, so any popup
        // content at those positions would remain if not cleared beforehand.
        let _ = execute!(
            stdout,
    ...
    // Restore cursor position (adjusted for any scrolling that was performed)
    let _ = execute!(
        stdout,
